...
interface VocabsQuizPracticeProps {
...
export default function VocabsQuizPractice({
  onBackToPracticeSelection,
}: VocabsQuizPracticeProps = {}) {
  ...
  const quizQuestions = useMemo(() => {
    ...
    return prioritizedWords.map((word) => {
      ...
      if (samePartOfSpeechWords.length >= 3) {
        ...
        const samePoSAnswers = samePartOfSpeechWords.map((w) => w.definition);
