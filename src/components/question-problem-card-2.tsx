...
const QuestionProblemCard = React.memo(function QuestionProblemCard({
  ...
  const [selectionPopover, setSelectionPopover] = useState<{
    /** Human-readable label shown in the popover chip */
    label: string;
    /** Raw text to copy — same as label for plain text, outer HTML for special nodes */
    copyText: string;
    x: number;
    y: number;
  } | null>(null);
  ...
  const handleTextSelection = useCallback((e: React.MouseEvent) => {
    ...
    // ── Detect a click/tap on a single non-text node (img, svg, mjx-container…) ──
    // When a user clicks such an element without dragging, the selection is
    // collapsed but the click target itself is the interesting node.
    const clickTarget = e.target as Element;
    const NON_TEXT_TAGS = ["IMG", "SVG", "MJX-CONTAINER", "MJX-MATH"];
    const nonTextEl =
      NON_TEXT_TAGS.includes(clickTarget.tagName.toUpperCase()) ||
      clickTarget.closest("mjx-container, svg")
        ? (clickTarget.closest("mjx-container, svg") ?? clickTarget)
        : null;

    if (nonTextEl) {
      const rect = nonTextEl.getBoundingClientRect();
      const tag = nonTextEl.tagName.toLowerCase();
      // For mjx-container use aria-label (MathJax sets this to the readable math expression)
      const mathLabel =
        nonTextEl.getAttribute("aria-label") ??
        nonTextEl.querySelector("[aria-label]")?.getAttribute("aria-label") ??
        `<${tag}>`;
      setSelectionPopover({
        label: `[${mathLabel}]`,
        copyText: nonTextEl.outerHTML,
    ...
    // Walk every node inside the range to find embedded special elements
    // and build a richer label when they are mixed in with text.
    const fragment = range.cloneContents();
    const specialNodes = fragment.querySelectorAll(
      "img, svg, mjx-container, mjx-math",
    );

    let label = text;
    if (specialNodes.length > 0) {
      // Build a compact display label: text interspersed with placeholders
      const parts: string[] = [];
      fragment.childNodes.forEach((node) => {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          if (tag === "mjx-container" || tag === "mjx-math") {
            const ariaLabel =
              el.getAttribute("aria-label") ??
              el.querySelector("[aria-label]")?.getAttribute("aria-label");
            parts.push(ariaLabel ? `[${ariaLabel}]` : "[math]");
          } else if (tag === "img") {
            const ariaLabel =
              el.getAttribute("aria-label") ?? el.getAttribute("alt");
            parts.push(ariaLabel ? `[${ariaLabel}]` : "[image]");
          } else if (tag === "svg") {
            parts.push("[svg]");
          } else {
  ...
  // Dismiss only when user clicks outside both the popover and the reading area
  useEffect(() => {
    const handlePointerDown = (e: PointerEvent) => {
      const insidePopover =
        selectionPopoverRef.current?.contains(e.target as Node) ?? false;
      const insideArea =
        selectionAreaRef.current?.contains(e.target as Node) ?? false;
      if (!insidePopover && !insideArea) {
    ...
    <React.Fragment>
      ...
      >
        ...
        <CardContent className="p-6">
          {/* Selectable reading area — popover fires on text selection */}
          <div ref={selectionAreaRef} onMouseUp={handleTextSelection}>
          ...
          {/* Selection popover — rendered in a portal so it sits above everything */}
          {selectionPopover &&
            ...
            createPortal(
              ...
              >
                ...
                <div className="flex justify-center">
                  <div className="w-2 h-2 bg-popover border-b border-r border-border rotate-45 translate-y-[5px] z-10" />
                </div>
                <div className="bg-popover text-popover-foreground border border-border rounded-lg shadow-lg px-3 py-2 flex items-center gap-2 text-sm whitespace-nowrap">
                  <span className="max-w-[200px] truncate text-muted-foreground font-medium">
                    &ldquo;{selectionPopover.label}&rdquo;
                  </span>
                  <button
                    className="flex items-center gap-1 px-2 py-1 rounded-md bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-colors"
                    onPointerDown={(e) => {
                      // Prevent the pointerdown dismiss handler from firing
                      e.stopPropagation();
                    }}
                    onClick={() => {
                      navigator.clipboard.writeText(selectionPopover.copyText);
